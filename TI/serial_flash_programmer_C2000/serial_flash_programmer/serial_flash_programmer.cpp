...
// Read a line from the serial port (similar to readline)
void read_serial_line(int serial_port, char* line, int max_length) {
    char buffer[1];   // Temporary buffer to read one byte at a time
    int line_index = 0;
    ssize_t num_bytes;

    while (true) {
        // Read one byte at a time
        num_bytes = read(serial_port, buffer, 1);

        if (num_bytes > 0) {
            line[line_index++] = buffer[0];

            // Stop when newline is encountered or when max length is reached
            if (buffer[0] == '\n' || line_index >= max_length - 1) {
...
// Self commented, as kernel is already loaded,
// Therefore no need to load kernel again
// #define kernel
...
//
//*****************************************************************************
//sudo ./serial_flash_programmer -p /dev/ttyUSB1 -b 9600 -d f2837xD

//sudo ./serial_flash_programmer -p /dev/ttyUSB1 -b 9600 -d f2837xD -k F2837xD_sci_flash_kernels_cpu01.txt -a blinky_dc_cpu01.txt -v
int _tmain(int argc, TCHAR *argv[])
{
	...
	// std::cout<<"COM PORT: "<<g_pszComPort<<std::endl;
	...
	if (g_bf021 == true && g_bf2837xD == true) //F021
	{
		...
		// Stuff for debugging
		char valToSend[30] = "Data from computer";
		char stringToReceive[50]; 
		const int max_line_length = 256;
    	char line[max_line_length]; // Initialize line buffer with a fixed size
		int ii = 0;
		int yy = 0;
		while (!done)
		{
			...
			_tprintf(_T("\t 0-DONE\n"));
			_tprintf(_T("\t 17-Debugging.\n"));
			_tprintf(_T("\t 18-AutoBaud lock\n"));
			uint32_t answer;
			cin >> dec >> answer;
			...
			case 1:
				...
				if (command != DFU_CPU1)
				{
					_tprintf(_T("ERROR with Packet Command!n"));
				}
				if (data[0] != NO_ERROR)
				{
					printErrorStatus(data[0]);
					// format mem addr output.
					// _tprintf(_T("ERROR Address: 0x: %s\n"), argv[iParm]);
					_tprintf(_T("\nERROR Address: 0x%.8X , 0x%.8X \n"),hex,formatMemAddr(data[2], data[1]) );
				...
				{
					_tprintf(_T("\nEntry Point Address is: 0x%.8X , 0x%.8x  \n"),hex,formatMemAddr(data[2], data[1]) );
					// cout << endl
			...
			case 2:
				...
				//Self inserted, might need to be deleted 
				//autobaudLock();


				if (!g_pszAppFile2)
				{
					_tprintf(_T("\nERROR: No flash application specified for CPU2 Flash Programming!\n"));
					ExitApp(3);
				}
				if (cpu2 == false)
				{
					_tprintf(_T("\nERROR: You must Boot_CPU2 before performing any operations on CPU2!\n"));
					break;
				}
				packetLength = constructPacket(packet, (uint16_t)DFU_CPU2, 0, NULL);
				_tprintf(_T("\ncalling f021_SendPacket with packetLength:%u\n"),packetLength);
				iRetCode = f021_SendPacket(packet, packetLength); //-1 means NACK, 0 means ACK
				_tprintf(_T("\nAfter sending packet, ret:%d\n"),iRetCode);
				iRetCode = f021_DownloadImage(g_pszAppFile2);
				_tprintf(_T("\nAfter f021_DownloadImage, ret:%d\n"),iRetCode);
				command = getPacket(&length, data);
				if (command != DFU_CPU2)
				{
			...
			case 11:
				if (cpu1 == false)
				{
					_tprintf(_T("\nERROR: Cannot perform operations on CPU1 after CPU2 is booted and given control of SCI!/n"));
					break;
				}
			...
			case 12:
				if (cpu1 == false)
				{
					_tprintf(_T("\nERROR: Cannot perform operations on CPU1 after CPU2 is booted and given control of SCI!\n"));
					break;
				}
			...
			case 13:
				if (!g_pszKernelFile2)
				{
					_tprintf(_T("\nERROR: No CPU2 flash kernel provided! "));
					ExitApp(3);
				}
				if (cpu1 == false)
				{
			...
			case 14:
			_tprintf(_T("\nRESET_CPU1_BOOT_CPU2 command found.\n"));
				if (!g_pszKernelFile2)
				{
					_tprintf(_T("\nERROR: No CPU2 flash kernel provided!!\n"));
					ExitApp(3);
				...
				//Send Kernel
				//_tprintf(_T("\ncalling f021_DownloadKernel CPU2 Kernel\n"));
				//iRetCode = f021_DownloadKernel(g_pszKernelFile2);// selfCommencted: ismail, 08.09.24s
				...
				// Do AutoBaud
				VERBOSEPRINT(_T("\nAttempting autobaud to send function message..."));
				// autobaudLock();// selfCommencted: ismail, 08.09.24
			...
			case 15:
				if (cpu2 == false)
				{
					_tprintf(_T("\nERROR: You must Boot_CPU2 before performing any operations on CPU2!/n"));
					break;
				}
				_tprintf(_T("\nPlease input a hexadecimal address to branch to:  "));
				cin >> hex >> branchAddress;
			...
			case 0:
				...
				_tprintf(_T("\nExiting the Application.n"));
				break;

			case 17:
				//Self inserted, might need to be deleted 
				//autobaudLock();
				_tprintf(_T("\nDebugging here on SCI-B boot.\n"));
				packetLength = constructPacket(packet, (uint16_t)0xCAFE, 0, NULL);
				_tprintf(_T("\ncalling f021_SendPacket, length:%d\n"),packetLength);
				iRetCode = f021_SendPacket(packet, packetLength); //-1 means NACK, 0 means ACK
				_tprintf(_T("\niRetCode: %d\n"),iRetCode);
				...
				for(ii=0;ii<10;ii++){
					...
					write(fd, valToSend, 20);

					read_serial_line(fd, line, max_line_length);
					
					for (yy = 0;yy<max_line_length;yy++){
						_tprintf(_T("%c,"),line[yy]);
						if (line[yy] == '\n')
							break;
					}
		...
		while (!done)
		{
			...
			case 8:
				...
				if (!g_pszAppFile)
				{
					...
					_tprintf(_T("\nERROR: No flash application specified for CPU1 Flash Programming!\n"));
					ExitApp(3);
				}
				if (cpu1 == false)
				{
					_tprintf(_T("\nERROR: Cannot perform operations on CPU1!\n"));
					break;
				}
...
void printErrorStatus(uint16_t status)
{
	...
	case PROGRAM_ERROR:
		...
		break;
	case COMMAND_ERROR:
		_tprintf(_T("\nERROR Status: COMMAND_ERROR\n"));
		break;
	case UNLOCK_ERROR:
		_tprintf(_T("\nERROR Status: UNLOCK_ERROR\n"));
		break;
	default:
